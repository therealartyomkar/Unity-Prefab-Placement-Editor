...
namespace Editor
{
    ...
    public class PrefabPlacementEditor : UnityEditor.Editor {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.Slider(size, 0.1f, 100, new GUIContent ("Size"));
        ...
        private void PrefabInstantiate (int index)
        {
            RaycastHit hit;
            ...
            if (instanceOf == null) throw new ArgumentNullException(nameof(instanceOf));
            ...
            {
                instanceOf.hideFlags = HideFlags.None;
            }

            instanceOf.transform.localScale = new Vector3 (prefabSize, prefabSize, prefabSize);
            instanceOf.transform.position = new Vector3(mousePos.x, mousePos.y + positionOffset.floatValue, mousePos.z);
            ...
            if (amount.intValue > 1)
            {
                ...
                {
                    if (!canPlaceOver.boolValue && hit.collider.CompareTag(instanceOf.tag))
                    {
                        DestroyImmediate(instanceOf);
                        return;
                    }

                    instanceOf.transform.position = new Vector3(hit.point.x, hit.point.y + positionOffset.floatValue, hit.point.z);
                    if(hideInHierarchy.boolValue)
        ...
        {
            var prefabsInRadius = GameObject.FindGameObjectsWithTag (prefabTag.stringValue);
